...
// use apsak_metrics_core::{Metrics,Metric};
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<ApsakRpcClient>> {
        self.rpc_api().clone().downcast_arc::<ApsakRpcClient>().ok()
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        log_trace!("Connected to apsakd: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");
