...
export interface IBalance {
    /**
     * Total amount of apsaK (in IPMOS) available for 
     * spending.
     */
    mature: bigint;
    /**
     * Total amount of apsaK (in IPMOS) that has been 
     * received and is pending confirmation.
     */
    pending: bigint;
    /**
     * Total amount of apsaK (in IPMOS) currently 
...
impl From<(Option<&Balance>, &NetworkType, Option<usize>)> for BalanceStrings {
    fn from((balance, network_type, padding): (Option<&Balance>, &NetworkType, Option<usize>)) -> Self {
        let suffix = utils::apsak_suffix(network_type);
        if let Some(balance) = balance {
            let mut mature = utils::ipmos_to_apsak_string(balance.mature);
            let mut pending = if balance.pending > 0 { Some(utils::ipmos_to_apsak_string(balance.pending)) } else { None };
