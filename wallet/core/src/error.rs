...
use apsak_bip32::Error as BIP32Error;
use apsak_consensus_core::sign::Error as CoreSignError;
use apsak_rpc_core::RpcError as apsaKRpcError;
use apsak_wrpc_client::error::Error as ApsakWorkflowRpcError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] apsak_wallet_keys::error::Error),

    #[error("please select an account")]
    AccountSelection,

    #[error("{0}")]
    ApsakRpcClientResult(#[from] apsaKRpcError),

    #[error("wRPC -> {0}")]
    RpcError(#[from] RpcError),

    #[error("Wallet wRPC -> {0}")]
    ApsakWorkflowRpcError(#[from] ApsakWorkflowRpcError),
    ...
    #[error("{0}")]
    NetworkId(#[from] apsak_consensus_core::network::NetworkIdError),
    ...
    #[error(transparent)]
    Address(#[from] apsak_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] apsak_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusWasm(#[from] apsak_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] apsak_txscript::MultisigCreateError),

    #[error(transparent)]
    TxScriptError(#[from] apsak_txscript_errors::TxScriptError),
