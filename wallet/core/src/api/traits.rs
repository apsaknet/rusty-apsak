//!
//! API trait for interfacing with the apsaK wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `apsak`.
    ///
    ...
    /// applications such as KDX and apsaknet.io web wallet using a deprecated derivation path
