...
pub fn get_apsak_tempdir() -> TempDir {
    let global_tempdir = std::env::temp_dir();
    let apsak_tempdir = global_tempdir.join("rusty-apsak");
    std::fs::create_dir_all(apsak_tempdir.as_path()).unwrap();
    let db_tempdir = tempfile::tempdir_in(apsak_tempdir.as_path()).unwrap();
    db_tempdir
}

/// Creates a DB within a temp directory under `<OS SPECIFIC TEMP DIR>/apsak-rust`
/// Callers must keep the `TempDbLifetime` guard for as long as they wish the DB to exist.
#[macro_export]
macro_rules! create_temp_db {
    ($conn_builder: expr) => {{
        let db_tempdir = $crate::utils::get_apsak_tempdir();
