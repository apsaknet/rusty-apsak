...
use apsak_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use apsak_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use apsak_core::{info, trace};
use apsak_database::prelude::{StoreError, StoreResult, DB};
use apsak_hashes::Hash;
use apsak_index_core::indexed_utxos::BalanceByScriptPublicKey;
use apsak_utils::arc::ArcExtensions;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-apsakd), for quicker resets, while still having a low memory footprint.
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<apsak_consensus_core::tx::TransactionOutpoint>> {
