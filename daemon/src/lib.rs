...
pub use apsakd::{Apsakd, ApsakdConfig, ApsakdCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../apsak-cpu-miner/target/debug",
    "../../apsak-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub apsakd: Option<Arc<dyn ApsakdCtl + Send + Sync + 'static>>,
    // pub apsakd_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { apsakd: None, cpu_miner: None }
    }

    pub fn with_apsakd(mut self, apsakd: Arc<dyn ApsakdCtl + Send + Sync + 'static>) -> Self {
        self.apsakd = Some(apsakd);
        self
    }
    ...
    pub fn apsakd(&self) -> Arc<dyn ApsakdCtl + Send + Sync + 'static> {
        self.apsakd.as_ref().expect("accessing Daemons::apsakd while apsakd option is None").clone()
    }

    pub fn try_apsakd(&self) -> Option<Arc<dyn ApsakdCtl + Send + Sync + 'static>> {
        self.apsakd.clone()
    }
