...
impl TryFrom<ApsakdConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: ApsakdConfig) -> Result<Vec<String>> {
...
impl Apsakd {
    pub fn new(args: ApsakdConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: ApsakdConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("apsaK node is already running.".to_string()));
            }
        }
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Apsakd binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}
...
pub trait ApsakdCtl {
    ...
    async fn configure(&self, config: ApsakdConfig) -> Result<()>;
