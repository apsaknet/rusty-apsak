...
*Testnet 11* is the test network running the first public 10BPS apsaK experiment. The goal of this document is to provide a quick guide for anyone who wants to participate.
...
1. *apsakd* - the apsaK client
2.  *apsak-miner* - the apsaK CPU miner
...
The recommended hardware requirements are 16GB of RAM, preferably a CPU with at least 8 cores, and an SSD drive with at least 250GB of free space (the 300GB for a safety margin is preferable). It might also be possible to operate with 8GB of RAM, by passing the `--ram-scale=0.6` parameter to `apsakd`.
...
1. Download and extract the [rusty-apsak binaries](https://github.com/apsaknet/rusty-apsak/releases). Alternatively, you can compile it from source yourself by following the instructions [here](https://github.com/apsaknet/rusty-apsak/blob/master/README.md). The rest of the instructions are written assuming the former option. If you choose to locally compile the code, replace any command of the form ``<program> <arguments>`` with ``cargo run --bin <program> --release -- <arguments>`` (see example in the next item). All actions described below should be performed on a command line window where you navigated to the directory into which the binaries were extracted.
2. Start the ``apsakd`` client with ``utxoindex`` enabled:

```
apsakd --testnet --netsuffix=11 --utxoindex
```
...
```
cargo run --bin apsakd --release -- --testnet --netsuffix=11 --utxoindex
```
...
2. The output will provide you with a private key (that looks like a bunch of gibberish) and a public address (that looks like "apsaktest:" followed by a bunch of gibberish). For example, the output could look like this:
     ```
     2023-06-25 18:00:58.677+00:00 [INFO ] Connected to RPC
     2023-06-25 18:00:58.677+00:00 [INFO ] Generated private key aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce and address apsaktest:qzlpwt49f0useql6w0tzpnf8k2symdv5tu2x2pe9r9nvngw8mvx57q0tt9lr5. Send some funds to this address and rerun rothschild with `--private-key aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce`
      ```
     Here, the private key is ```aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce``` and the address is ```apsaktest:qzlpwt49f0useql6w0tzpnf8k2symdv5tu2x2pe9r9nvngw8mvx57q0tt9lr5```
...
Like apsakd, the Rothschild window should remain open and undisturbed.

For mining, grab `apsak-miner` from within the latest [Release](https://github.com/elichai/apsak-miner/releases) and run it with the following flags (**this is currently the only miner that supports testnet-11**):
    ```
    apsak-miner --testnet --mining-address <address> -p 16210 -t 1
    ```
...
Like the Apsakd and Rothschild windows, the miner window should also be left undisturbed, and closing it will stop the mining.
