...
use apsak_consensus_core::{blockhash::BlockHashes, BlockHashMap, BlockHasher, BlockLevel};
use apsak_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use apsak_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use apsak_database::prelude::{DirectWriter, MemoryWriter};
use apsak_database::prelude::{ReadLock, StoreError};
use apsak_database::prelude::{StoreResult, DB};
use apsak_database::registry::{DatabaseStorePrefixes, SEPARATOR};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; apsak_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use apsak_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(apsak_database::prelude::ConnBuilder::default().with_files_limit(10));
