use crate::constants::{MAX_IPMOS, SEQUENCE_LOCK_TIME_DISABLED, SEQUENCE_LOCK_TIME_MASK};
use apsak_consensus_core::{hashing::sighash::SigHashReusedValues, tx::VerifiableTransaction};
use apsak_core::warn;
use apsak_txscript::{get_sig_op_count, TxScriptEngine};
    ...
    fn check_sequence_lock(tx: &impl VerifiableTransaction, pov_daa_score: u64) -> TxResult<()> {
        ...
        if tx.populated_inputs().filter(|(input, _)| input.sequence & SEQUENCE_LOCK_TIME_DISABLED != SEQUENCE_LOCK_TIME_DISABLED).any(
            |(input, entry)| {
                ...
                //
                // Note: in the apsakd codebase there's a use in i64 in order to use the -1 value
...
mod tests {
    ...
    use apsak_consensus_core::tx::{MutableTransaction, PopulatedTransaction, ScriptVec, TransactionId, UtxoEntry};
    use apsak_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput};
