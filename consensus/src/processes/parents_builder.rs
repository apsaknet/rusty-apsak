...
use apsak_consensus_core::{blockhash::ORIGIN, header::Header, BlockHashMap, BlockHasher, BlockLevel};
...
mod tests {
    ...
    use apsak_database::prelude::{ReadLock, StoreError, StoreResult};
    use apsak_hashes::Hash;
    ...
    impl HeaderStoreReader for HeaderStoreMock {
        fn get_daa_score(&self, hash: apsak_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_timestamp(&self, hash: apsak_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_bits(&self, hash: apsak_hashes::Hash) -> Result<u32, StoreError> {
            unimplemented!()
        }

        fn get_header(&self, hash: apsak_hashes::Hash) -> Result<Arc<Header>, StoreError> {
            Ok(self.map.read().get(&hash).unwrap().header.clone())
        }

        fn get_compact_header_data(
            &self,
            hash: apsak_hashes::Hash,
        ) -> Result<crate::model::stores::headers::CompactHeaderData, StoreError> {
            unimplemented!()
        }

        fn get_blue_score(&self, hash: apsak_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_header_with_block_level(&self, hash: apsak_hashes::Hash) -> Result<HeaderWithBlockLevel, StoreError> {
    ...
    impl RelationsStoreReader for RelationsStoreMock {
        fn get_parents(&self, hash: Hash) -> Result<apsak_consensus_core::blockhash::BlockHashes, StoreError> {
            unimplemented!()
        }
