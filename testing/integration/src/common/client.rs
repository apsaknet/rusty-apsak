...
use apsak_notify::{events::EventType, scope::Scope, subscription::Command};
use apsak_rpc_core::RpcResult;
use std::{
...
impl ListeningClient {
    pub async fn connect(apsakd: &Daemon) -> Self {
        let client = apsakd.new_multi_listener_client().await;
