...
use apsak_wallet_core::storage::{make_filename, Hint};

pub(crate) async fn create(ctx: &Arc<ApsakCli>, name: Option<&str>, import_with_mnemonic: bool) -> Result<()> {
    let term = ctx.term();
    ...
    if let Err(err) = wallet.network_id() {
        tprintln!(ctx);
        tprintln!(ctx, "Before creating a wallet, you need to select a apsaK network.");
