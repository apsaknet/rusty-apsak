...
use apsak_bip32::secp256k1::XOnlyPublicKey;
use apsak_wallet_core::{
...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<ApsakCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
        ...
        match argv.first().unwrap().as_str() {
            ...
            "verify" => {
                ...
                self.verify(ctx, apsak_address, signature, message).await?;
            }
            v => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<ApsakCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("sign <apsak_address>", "Sign a message with the private key that matches the given address. Prompts for message."),
                (
                    "verify <apsak_address> <signature>",
                    "Verify the signature against the message and apsak_address. Prompts for message.",
                ),
            ],
            None,
        )?;
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<ApsakCli>, apsak_address: &str, message: &str) -> Result<()> {
        let apsak_address = Address::try_from(apsak_address)?;
        if apsak_address.version != Version::PubKey {
        ...
        let privkey = self.get_address_private_key(&ctx, apsak_address).await?;
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<ApsakCli>, apsak_address: &str, signature: &str, message: &str) -> Result<()> {
        let apsak_address = Address::try_from(apsak_address)?;
        if apsak_address.version != Version::PubKey {
            return Err(Error::custom("Address not supported for message signing. Only supports PubKey addresses"));
        }

        let pubkey = XOnlyPublicKey::from_slice(&apsak_address.payload[0..32]).unwrap();
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<ApsakCli>, apsak_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind().as_ref() {
            BIP32_ACCOUNT_KIND => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&apsak_address])?;
