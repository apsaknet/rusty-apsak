...
pub struct Node {
    settings: SettingsStore<ApsakdSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("apsakd").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<ApsakCli>() {
            ctx.daemons().clone().apsakd.as_ref().map(|_| "node")
        } else {
            None
        }
    }
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<ApsakCli>) -> Result<ApsakdConfig> {
        let location: String = self
            .settings
            .get(ApsakdSettings::Location)
        ...
        let mute = false;
        let config = ApsakdConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<ApsakCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let apsakd = ctx.daemons().apsakd();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting apsak node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting apsak node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                apsakd.configure(self.create_config(&ctx).await?).await?;
                apsakd.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // apsakd.mute(mute).await?;
                self.settings.set(ApsakdSettings::Mute, mute).await?;
            }
            "status" => {
                let status = apsakd.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                apsakd.configure(self.create_config(&ctx).await?).await?;
                let version = apsakd.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<ApsakCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select Apsakd executable (binary) location"),
                ("version", "Display Apsakd executable version"),
                ("start", "Start the local apsaK node instance"),
                ("stop", "Stop the local apsaK node instance"),
                ("restart", "Restart the local apsaK node instance"),
                ("kill", "Kill the local apsaK node instance"),
                ("status", "Get the status of the local apsaK node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<ApsakCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = apsak_daemon::locate_binaries(root.as_str(), "apsakd").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No apsakd binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a apsakd binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(ApsakdSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(ApsakdSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/apsakd'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<ApsakCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "Apsakd has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("Apsakd error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
