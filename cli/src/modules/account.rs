use apsak_wallet_core::account::BIP32_ACCOUNT_KIND;
use apsak_wallet_core::account::LEGACY_ACCOUNT_KIND;
        ...
        match action.as_str() {
            ...
            "import" => {
                if argv.is_empty() {
                    ...
                    ctx.term().help(
                        &[
                            ("account import legacy-data", "Import KDX keydata file or apsaknet web wallet data on the same domain"),
                            (
                                "account import mnemonic bip32",
                                "Import Bip32 (12 or 24 word mnemonics used by apsakwallet, kaspium, onekey, tangem etc.)",
                            ),
                            (
                                "account import mnemonic legacy",
                                "Import accounts 12 word mnemonic used by legacy applications (KDX and apsaknet web wallet)",
                            ),
                            (
                ...
                match import_kind.as_ref() {
                    "legacy-data" => {
                        ...
                        if exists_legacy_v0_keydata().await? {
                                ...
                                .import_legacy_keydata(
                                    ...
                                    Some(Arc::new(move |processed: usize, _, balance, txid| {
                                        if let Some(txid) = txid {
                                            tprintln!(
                                                ctx_,
                                                "Scan detected {} SAK at index {}; transfer txid: {}",
                                                ipmos_to_apsak_string(balance),
                                                processed,
                                                txid
                                            );
                                        } else if processed > 0 {
                            ...
                            return Err("'apsaknet' web wallet storage not found at this domain name".into());
                        } else {
                    ...
                    "mnemonic" => {
                        if argv.is_empty() {
                            ...
                            tprintln!(ctx, "please use 'legacy' for 12-word KDX and apsaknet web wallet mnemonics\r\n");
                            return Ok(());
                        }
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<ApsakCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                (KDX and apsaknet web wallet). Use 'account import' for additional help.",
                ),
            ...
            .derivation_scan(
                ...
                Some(Arc::new(move |processed: usize, _, balance, txid| {
                    if let Some(txid) = txid {
                        ...
                        tprintln!(ctx_, "Scanned {} derivations, found {} SAK", processed, ipmos_to_apsak_string(balance));
                    }
                })),
            )
            .await?;
