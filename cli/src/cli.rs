...
use apsak_daemon::{DaemonEvent, DaemonKind, Daemons};
use apsak_wallet_core::rpc::DynRpcApi;
use apsak_wallet_core::storage::{IdT, PrvKeyDataInfo};
use apsak_wrpc_client::ApsakRpcClient;
...
impl From<&ApsakCli> for Arc<Terminal> {
    fn from(ctx: &ApsakCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<ApsakCli> for ApsakCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for ApsakCli {
...
impl ApsakCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                apsak_core::log::init_logger(None, "info");
            } else {
                apsak_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let apsak_cli = Arc::new(ApsakCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(apsak_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                apsak_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<ApsakRpcClient>> {
                ...
                select! {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletPing => {
                                    // log_info!("apsaK NG - received wallet ping");
                                },
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: apsaK node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to apsaK node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - apsaK node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "apsaK node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let apsakd = self.daemons().try_apsakd();
            ...
            if let Some(apsakd) = apsakd.as_ref() {
                apsakd.mute(false).await?;
                apsakd.stop().await?;
            }
...
impl Cli for ApsakCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "apsak" {
                prompt.push(title);
            }
...
pub async fn apsak_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    ApsakCli::init();

    let options = Options::new(terminal_options, None);
    let cli = ApsakCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("apsaK Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
