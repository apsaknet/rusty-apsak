...
pub enum GrpcServerError {
    ...
    RpcApiError(#[from] apsak_rpc_core::error::RpcError),
    ...
    ClosedHandler(ApsakdPayloadOps),
...
impl From<GrpcServerError> for apsak_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => apsak_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for apsak_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => apsak_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => apsak_notify::error::Error::General(err.to_string()),
        }
    }
}
...
impl<T> From<TrySendError<T>> for GrpcServerError {
    fn from(_: TrySendError<T>) -> Self {
        apsak_notify::error::Error::ChannelSendError.into()
    }
}
