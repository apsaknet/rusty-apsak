...
use super::{
    ...
    interface::{Interface, ApsakdMethod, ApsakdRoutingPolicy},
    method::Method,
};
use crate::{
...
use apsak_grpc_core::protowire::{apsakd_request::Payload, *};
use apsak_grpc_core::{ops::ApsakdPayloadOps, protowire::NotifyFinalityConflictResponseMessage};
use apsak_notify::{scope::FinalityConflictResolvedScope, subscriber::SubscriptionManager};
use apsak_rpc_core::{SubmitBlockRejectReason, SubmitBlockReport, SubmitBlockResponse};
use apsak_rpc_macros::build_grpc_server_interface;

pub struct Factory {}

impl Factory {
    pub fn new_handler(
        rpc_op: ApsakdPayloadOps,
    ...
    pub fn new_interface(server_ctx: ServerContext, network_bps: u64) -> Interface {
        ...
        let method: ApsakdMethod = Method::new(|server_ctx: ServerContext, connection: Connection, request: ApsakdRequest| {
            Box::pin(async move {
                let mut response: ApsakdResponse = match request.payload {
                    Some(Payload::NotifyFinalityConflictRequest(ref request)) => {
                        match apsak_rpc_core::NotifyFinalityConflictRequest::try_from(request) {
                            Ok(request) => {
        ...
        interface.replace_method(ApsakdPayloadOps::NotifyFinalityConflict, method);
        ...
        interface.set_method_properties(
            ...
            ApsakdRoutingPolicy::DropIfFull(Arc::new(Box::new(|_: &ApsakdRequest| {
