...
use apsak_notify::scope::{NewBlockTemplateScope, Scope};
use apsak_rpc_core::api::rpc::RpcApi;
use apsak_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    apsak_core::log::try_init_logger("info, apsak_grpc_core=trace, apsak_grpc_server=trace, apsak_grpc_client=trace");
