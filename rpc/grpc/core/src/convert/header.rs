...
use apsak_rpc_core::{FromRpcHex, RpcError, RpcHash, RpcResult, ToRpcHex};
use std::str::FromStr;
...
from!(item: &apsak_rpc_core::RpcHeader, protowire::RpcBlockHeader, {
    Self {
...
try_from!(item: &protowire::RpcBlockHeader, apsak_rpc_core::RpcHeader, {
    ...
    Self::new_finalized(
        ...
        item.nonce,
        item.daa_score,
        apsak_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        item.blue_score,
...
mod tests {
    use crate::protowire;
    use apsak_rpc_core::{RpcHash, RpcHeader};
