...
use apsak_core::{debug, error, trace};
use apsak_grpc_core::{
    channel::NotificationChannel,
    ops::ApsakdPayloadOps,
    protowire::{apsakd_request, rpc_client::RpcClient, GetInfoRequestMessage, ApsakdRequest, ApsakdResponse},
...
use apsak_utils::{channel::Channel, triggers::DuplexTrigger};
use apsak_utils_tower::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(ApsakdPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type ApsakdRequestSender = async_channel::Sender<ApsakdRequest>;
type ApsakdRequestReceiver = async_channel::Receiver<ApsakdRequest>;
...
/// Data flow:
/// ```
/// //   ApsakdRequest -> request_send -> stream -> ApsakdResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: ApsakdRequestSender,
    request_receiver: ApsakdRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<ApsakdResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual ApsakdRequest to ApsakdResponse stream
        let mut stream: Streaming<ApsakdResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: ApsakdPayloadOps, request: impl Into<ApsakdRequest>) -> Result<ApsakdResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: ApsakdRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<ApsakdResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = apsakd_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = apsakd_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
