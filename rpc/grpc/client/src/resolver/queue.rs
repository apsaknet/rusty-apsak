use crate::{
    error::{Error, Result},
    resolver::{matcher::Matcher, ApsakdResponseReceiver, ApsakdResponseSender, Resolver},
};
...
struct Pending {
    ...
    sender: ApsakdResponseSender,
}

impl Pending {
    fn new(op: ApsakdPayloadOps, request: ApsakdRequest, sender: ApsakdResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &ApsakdResponse, response_op: ApsakdPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: ApsakdPayloadOps, request: &ApsakdRequest) -> ApsakdResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<ApsakdResponse>>();
        {
    ...
    fn handle_response(&self, response: ApsakdResponse) {
        let response_op: ApsakdPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");
