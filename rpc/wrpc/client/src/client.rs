...
pub use apsak_rpc_macros::build_wrpc_client_interface;
use std::fmt::Debug;
...
impl Inner {
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        // log_trace!("apsaK wRPC::{encoding} connecting to: {url}");
        ...
        .for_each(|notification_op| {
            ...
            interface.notification(
                notification_op,
                workflow_rpc::client::Notification::new(move |notification: apsak_rpc_core::Notification| {
                    ...
                    Box::pin(async move {
                        ...
                        if notification_sender.receiver_count() > 1 {
                            ...
                            log_warn!("WARNING: apsaK RPC notification is not consumed by user: {:?}", notification);
                        }
                        Ok(())
                    })
                }),
...
/// [`ApsakRpcClient`] allows connection to the apsaK wRPC Server via
...
///
#[derive(Clone)]
pub struct ApsakRpcClient {
    inner: Arc<Inner>,
}

impl Debug for ApsakRpcClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ApsakRpcClient").field("url", &self.url()).field("connected", &self.is_connected()).finish()
    }
}

impl ApsakRpcClient {
    /// Create a new `ApsakRpcClient` with the given Encoding and URL
    pub fn new(
        encoding: Encoding,
        url: Option<&str>,
    ...
    ) -> Result<ApsakRpcClient> {
        Self::new_with_args(encoding, url, resolver, network_id, subscription_context)
        // FIXME
        // pub fn new(encoding: Encoding, url: &str, ) -> Result<ApsakRpcClient> {
    ...
    pub fn new_with_args(
        ...
        let client = ApsakRpcClient { inner };
        ...
        // let client = ApsakRpcClient { inner, notifier, notification_mode };
            ...
            loop {
                select! {
                    ...
                    msg = notification_relay_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            // inner.rpc_ctl.notify(msg).await.expect("(ApsakRpcClient) rpc_ctl.notify() error");
                    ...
                    msg = wrpc_ctl_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            match msg {
                                WrpcCtl::Connect => {
                                    inner.rpc_ctl.signal_open().await.expect("(ApsakRpcClient) rpc_ctl.signal_open() error");
                                }
                                WrpcCtl::Disconnect => {
                                    inner.rpc_ctl.signal_close().await.expect("(ApsakRpcClient) rpc_ctl.signal_close() error");
                                }
                            }
                        } else {
...
#[async_trait]
impl RpcApi for ApsakRpcClient {
    //
