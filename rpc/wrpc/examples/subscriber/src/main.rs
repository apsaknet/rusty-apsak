// Basic example of a apsaK wRPC client that connects to a node
...
// apsaK RPC primitives
use apsak_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use apsak_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic apsaK RPC client instance using Borsh encoding.
        let client = Arc::new(ApsakRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
