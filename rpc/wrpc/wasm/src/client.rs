...
use apsak_addresses::{Address, AddressOrStringArrayT};
use apsak_consensus_client::UtxoEntryReference;
use apsak_consensus_core::network::{NetworkType, NetworkTypeT};
use apsak_notify::connection::ChannelType;
use apsak_notify::events::EventType;
use apsak_notify::listener;
use apsak_notify::notification::Notification as NotificationT;
use apsak_rpc_core::api::ctl;
pub use apsak_rpc_core::wasm::message::*;
pub use apsak_rpc_macros::{
    build_wrpc_wasm_bindgen_interface, build_wrpc_wasm_bindgen_subscriptions, declare_typescript_wasm_interface as declare,
};
use apsak_wasm_core::events::{get_event_targets, Sink};
...
pub struct Inner {
    ...
    notification_channel: Channel<apsak_rpc_core::Notification>,
}
...
///
///
/// apsaK RPC client uses ([wRPC](https://github.com/workflow-rs/workflow-rs/tree/master/rpc))
/// interface to connect directly with apsaK Node. wRPC supports
/// two types of encodings: `borsh` (binary, default) and `json`.
///
/// There are two ways to connect: Directly to any apsaK Node or to a
...
/// ```
///
/// **Connecting to a apsaK Node directly**
///
/// ```javascript
/// let rpc = new RpcClient({
...
/// to re-register event listeners, but your have to re-subscribe for apsaK node
/// notifications:
///
/// ```typescript
/// rpc.addEventListener("connect", async (event) => {
...
impl RpcClient {
    pub fn new(config: Option<RpcConfig>) -> Result<RpcClient> {
        ...
        let client = Arc::new(
            ApsakRpcClient::new(encoding, url.as_deref(), resolver.clone().map(Into::into), network_id, None)
...
impl RpcClient {
    ...
    /// Connect to the apsaK RPC server. This function starts a background
    ...
    /// Disconnect from the apsaK RPC server.
    ...
    /// however, you have to re-subscribe for apsaK node notifications. As such, it is recommended
...
impl RpcClient {
    pub fn new_with_rpc_client(client: Arc<ApsakRpcClient>) -> RpcClient {
    ...
    pub fn client(&self) -> &Arc<ApsakRpcClient> {
        &self.inner.client
    }
                ...
                select_biased! {
                    ...
                    msg = notification_receiver.recv().fuse() => {
                        if let Ok(notification) = &msg {
                            match &notification {
                                apsak_rpc_core::Notification::UtxosChanged(utxos_changed_notification) => {
...
impl RpcClient {
    ...
    pub fn parse_url(url: &str, encoding: Encoding, network: NetworkId) -> Result<String> {
        let url_ = ApsakRpcClient::parse_url(url.to_string(), encoding, network.into())?;
        Ok(url_)
    }
}
...
impl RpcClient {
    ...
    /// Difficulty Adjustment Algorithm (DAA) score changes in the apsaK BlockDAG.
    ...
    /// apsaK BlockDAG. The event notification will be scoped to the
    ...
    /// chain changes in the apsaK BlockDAG.
...
build_wrpc_wasm_bindgen_subscriptions!([
    ...
    /// block is added to the apsaK BlockDAG.
    BlockAdded,
    ...
    /// conflict occurs in the apsaK BlockDAG.
    FinalityConflict,
    ...
    /// conflict in the apsaK BlockDAG is resolved.
    ...
    /// score of the sink block changes in the apsaK BlockDAG.
    ...
    /// UTXO set override for the pruning point changes in the apsaK BlockDAG.
    ...
    /// template is generated for mining in the apsaK BlockDAG.
    NewBlockTemplate,
]);
...
build_wrpc_wasm_bindgen_interface!(
    [
        ...
        /// Retrieves the current number of blocks in the apsaK BlockDAG.
        ...
        /// structure of the apsaK BlockDAG.
        ...
        GetBlockDagInfo,
        /// Returns the total current coin supply of apsaK network.
        /// Returned information: Total coin supply.
        GetCoinSupply,
        /// Retrieves information about the peers connected to the apsaK node.
        /// Returned information: Peer ID, IP address and port, connection
        /// status, protocol version.
        GetConnectedPeerInfo,
        /// Retrieves general information about the apsaK node.
        /// Returned information: Version of the apsaK node, protocol
        /// version, network identifier.
        /// This call is primarily used by gRPC clients.
        /// For wRPC clients, use {@link RpcClient.getServerInfo}.
        GetInfo,
        /// Provides a list of addresses of known peers in the apsaK
        ...
        GetSinkBlueScore,
        /// Tests the connection and responsiveness of a apsaK node.
        /// Returned information: None.
        Ping,
        /// Gracefully shuts down the apsaK node.
        /// Returned information: None.
        Shutdown,
        /// Retrieves information about the apsaK server.
        /// Returned information: Version of the apsaK server, protocol
        /// version, network identifier.
        GetServerInfo,
        /// Obtains basic information about the synchronization status of the apsaK node.
        /// Returned information: Syncing status.
        GetSyncStatus,
    ],
    [
        // functions with `request` argument
        /// Adds a peer to the apsaK node's list of known peers.
        /// Returned information: None.
        AddPeer,
        /// Bans a peer from connecting to the apsaK node for a specified duration.
        /// Returned information: None.
        Ban,
        ...
        /// Retrieves the balance of a specific address in the apsaK BlockDAG.
        /// Returned information: Balance of the address.
        GetBalanceByAddress,
        /// Retrieves balances for multiple addresses in the apsaK BlockDAG.
        /// Returned information: Balances of the addresses.
        GetBalancesByAddresses,
        /// Retrieves a specific block from the apsaK BlockDAG.
        /// Returned information: Block information.
        GetBlock,
        /// Retrieves multiple blocks from the apsaK BlockDAG.
        ...
        GetCurrentNetwork,
        /// Retrieves block headers from the apsaK BlockDAG.
        /// Returned information: List of block headers.
        GetHeaders,
        /// Retrieves mempool entries from the apsaK node's mempool.
        ...
        GetMempoolEntry,
        /// Retrieves information about a subnetwork in the apsaK BlockDAG.
        ...
        /// Resolves a finality conflict in the apsaK BlockDAG.
        /// Returned information: None.
        ResolveFinalityConflict,
        /// Submits a block to the apsaK network.
        /// Returned information: None.
        SubmitBlock,
        /// Submits a transaction to the apsaK network.
        /// Returned information: None.
        SubmitTransaction,
        /// Unbans a previously banned peer, allowing it to connect
        /// to the apsaK node again.
        /// Returned information: None.
        Unban,
    ]
);
